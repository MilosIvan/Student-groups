...
public class DodajProjektController {
    ...
    public void dodajProjektButton() {
        try {
            String naziv;
            ...
            dialog.setHeaderText("Dodaj projekt");
            ...
            if(result.isPresent()) {
                if (result.get() == ButtonType.OK) {
                    Projekt projekt = new Projekt(0, naziv, klubOrganizator, grad, datumPocetka, datumZavrsetka, tema);

                    Database.dodajProjekt(projekt);

                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                    alert.setTitle("Pokušaj uspješan");
                    alert.setHeaderText("Uspješno ste dodali novi projekt");
                    alert.showAndWait();

                    Promjena promjena = new Promjena(
                            "Dodan projekt " + projekt.getNaziv(),
                            projekt.getId(),
            ...
            ApplicationProject.logger.error("Nepotpun unos podataka prilikom dodavanja novog projekta");
