...
public class DodajPutovanjeController {
    @FXML
    ...
    @FXML
    private TextField povodPutovanjaTextField;
    ...
    public void dodajPutovanjeButton() {
        try {
            String naziv;
            ...
            if((povodPutovanja = povodPutovanjaTextField.getText()).isEmpty()) {
                throw new NepotpunUnosPodatakaException("povod putovanja");
            }
            ...
            dialog.setHeaderText("Dodaj putovanje");
            ...
            if(result.isPresent()) {
                if (result.get() == ButtonType.OK) {
                    Putovanje putovanje = new Putovanje(0, naziv, klubOrganizator, grad, datumPocetka, datumZavrsetka, povodPutovanja);

                    Database.dodajPutovanje(putovanje);

                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                    alert.setTitle("Pokušaj uspješan");
                    alert.setHeaderText("Uspješno ste dodali novo putovanje");
                    alert.showAndWait();

                    Promjena promjena = new Promjena(
                            "Dodano putovanje " + putovanje.getNaziv(),
                            putovanje.getId(),
            ...
            ApplicationProject.logger.error("Nepotpun unos podataka prilikom dodavanja novog putovanja");
